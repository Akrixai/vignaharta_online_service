...
async function processKwikAPICallback(data: any) {
  try {
    ...
    if (newStatus === 'SUCCESS' && previousStatus !== 'SUCCESS') {
      console.log('💰 [Callback.php] Processing SUCCESS settlement for transaction:', transaction.id);
      await processTransactionSuccess(transaction);
    } else if (newStatus === 'FAILED') {
      console.log('�️ [Callback.php] Deleting failed transaction from history:', transaction.id);
      await supabase
        .from('recharge_transactions')
        .delete()
        .eq('id', transaction.id);
...
async function processTransactionSuccess(transaction: any) {
  try {
    ...
    console.log(`✅ [Callback.php] Settlement completed for ${transaction.id}`);
  } catch (error) {
    console.error('❌ [Callback.php] Success settlement error:', error);
  }
}
