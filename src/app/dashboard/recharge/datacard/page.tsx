...
export default function DataCardPage() {
  ...
  const fetchOperators = async () => {
    try {
      const res = await fetch('/api/recharge/operators?service_type=DATACARD');
      const data = await res.json();
      if (data.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Get the primary identifier from dynamic fields or fallback to dataCardNumber
    let primaryNumber = '';
    ...
    try {
      ...
      // Use KwikAPI recharge endpoint for DataCard
      const payload: any = {
      ...
      if (data.success) {
        ...
        if (status === 'SUCCESS') {
          ...
          const finalSuccessMsg = `✅ ${cleanMessage}! Your data card recharge for ${primaryNumber} was successful.${operatorRef ? `\nRef: ${operatorRef}` : ''}`;
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8 max-w-7xl">
        <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 text-gray-800">📶 Data Card Recharge</h1>

        {/* Header */}
        <div className="mb-4 sm:mb-6">
          <div className="bg-cyan-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold text-sm sm:text-base text-center">
            📶 DATA CARD RECHARGE
          </div>
        </div>
        ...
        {currentStep === 1 ? (
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            <h2 className="text-xl font-bold mb-4 text-gray-800">Step 1: Select Your Data Card Provider</h2>
            
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Select Data Card Provider
              </label>
              <SearchableSelect
                ...
                placeholder="Search and select your data card provider..."
                required
              />
            </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-800">Step 2: Enter Recharge Details</h2>
              <button
                onClick={() => {
                  setCurrentStep(1);
                  setOperatorSelected(false);
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                {/* Fallback Data Card Number field - only show if no dynamic fields */}
              ...
              <button
                ...
                className="w-full py-4 bg-cyan-600 text-white font-semibold rounded-lg hover:bg-cyan-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all shadow-lg"
              >
                {loading ? '⏳ Processing Recharge...' : `🚀 Recharge Data Card - ₹${amount || '0'}`}
              </button>
