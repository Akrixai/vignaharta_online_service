...
export default function GasBillPage() {
  ...
  const fetchOperators = async () => {
    try {
      const res = await fetch('/api/recharge/operators?service_type=GAS');
      const data = await res.json();
      if (data.success) {
  ...
  const fetchBill = async () => {
    // Get the primary identifier from dynamic fields or fallback to consumerNumber
    let primaryNumber = '';
    ...
    if (dynamicFields.length > 0) {
      ...
      const primaryField = dynamicFields.find(field => 
        field.required && (
          field.name.toLowerCase().includes('consumer') ||
      ...
      primaryNumber = consumerNumber;
      if (!primaryNumber) {
        setMessage('Please enter consumer number');
        setMessageType('error');
        return;
      }
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        if (status === 'SUCCESS') {
          ...
          const finalSuccessMsg = `✅ ${cleanMessage}! Your gas bill payment for ${primaryNumber} was successful.${operatorRef ? `\nRef: ${operatorRef}` : ''}`;
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8 max-w-7xl">
        <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 text-gray-800">🔥 Gas Bill Payment</h1>

        {/* Header */}
        <div className="mb-4 sm:mb-6">
          <div className="bg-orange-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold text-sm sm:text-base text-center">
            🔥 GAS BILL PAYMENT
          </div>
        </div>
        ...
        {currentStep === 1 ? (
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            <h2 className="text-xl font-bold mb-4 text-gray-800">Step 1: Select Your Gas Provider</h2>
            ...
            <div className="mb-6">
              ...
              <SearchableSelect
                ...
                placeholder="Search and select your gas provider..."
                required
              />
            </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                ...
                {dynamicFields.length === 0 && (
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      placeholder="Enter your gas consumer number"
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                      required
                    />
                  </div>
              ...
              <button
                ...
                className="w-full py-4 bg-orange-600 text-white font-semibold rounded-lg hover:bg-orange-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all shadow-lg"
              >
                {loading ? '⏳ Processing Payment...' : `🚀 Pay Gas Bill - ₹${amount || '0'}`}
              </button>
