...
export default function CableTVPage() {
  ...
  const fetchOperators = async () => {
    try {
      const res = await fetch('/api/recharge/operators?service_type=CableTV');
      const data = await res.json();
      if (data.success) {
  ...
  const fetchBill = async () => {
    // Get the primary identifier from dynamic fields or fallback to subscriberNumber
    let primaryNumber = '';
    ...
    if (dynamicFields.length > 0) {
      ...
      const primaryField = dynamicFields.find(field => 
        field.required && (
          field.name.toLowerCase().includes('subscriber') ||
      ...
      primaryNumber = subscriberNumber;
      if (!primaryNumber) {
        setMessage('Please enter subscriber number');
        setMessageType('error');
        return;
      }
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        if (status === 'SUCCESS') {
          ...
          const finalSuccessMsg = `✅ ${cleanMessage}! Your cable TV bill payment for ${primaryNumber} was successful.${operatorRef ? `\nRef: ${operatorRef}` : ''}`;
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8 max-w-7xl">
        <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 text-gray-800">📡 Cable TV Bill Payment</h1>

        {/* Header */}
        <div className="mb-4 sm:mb-6">
          <div className="bg-violet-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold text-sm sm:text-base text-center">
        ...
        {currentStep === 1 ? (
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            <h2 className="text-xl font-bold mb-4 text-gray-800">Step 1: Select Your Cable TV Provider</h2>
            ...
            <div className="mb-6">
              ...
              <SearchableSelect
                ...
                placeholder="Search and select your cable TV provider..."
                required
              />
            </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                {/* Fallback Subscriber Number field - only show if no dynamic fields */}
                {dynamicFields.length === 0 && (
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      placeholder="Enter your cable TV subscriber number"
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 focus:border-transparent"
                      required
                    />
                  </div>
                ...
                <div className="flex items-end">
                  <button
                    type="button"
                    onClick={fetchBill}
                    disabled={fetchingBill || (!subscriberNumber && dynamicFields.length === 0) || !selectedOperator}
              ...
              <button
                ...
                className="w-full py-4 bg-violet-600 text-white font-semibold rounded-lg hover:bg-violet-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all shadow-lg"
              >
                {loading ? '⏳ Processing Payment...' : `🚀 Pay Cable TV Bill - ₹${amount || '0'}`}
              </button>
