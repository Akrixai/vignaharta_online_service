...
export default function InsurancePremiumPage() {
  ...
  const fetchOperators = async () => {
    try {
      const res = await fetch('/api/recharge/operators?service_type=INSURANCE');
      const data = await res.json();
      if (data.success) {
  ...
  const fetchBill = async () => {
    ...
    if (operator?.metadata?.bill_fetch !== 'YES') {
      setMessage('⚠️ Premium fetch not supported for this insurance company. Please enter the amount manually.');
      setMessageType('info');
      return;
    }

    setFetchingBill(true);
    setMessage('🔍 Fetching your premium details...');
    setMessageType('info');
    setBillDetails(null);
    ...
    try {
      ...
      console.log('📦 [Frontend] Premium fetch response:', data);
      ...
      if (data.success) {
        ...
        setMessage(`✅ Premium found for ${billData.customer_name || billData.customername || 'customer'} - Amount: ₹${billData.due_amount || billData.dueamount}`);
        setMessageType('success');
      } else {
        setMessage(data.message || 'Unable to fetch premium details');
        setMessageType('error');
      }
    } catch (error: any) {
      setMessage(`❌ Error fetching premium: ${error.message}`);
      setMessageType('error');
    } finally {
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8 max-w-7xl">
        <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 text-gray-800">🛡️ Insurance Premium Payment</h1>
        ...
        {currentStep === 1 ? (
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            <h2 className="text-xl font-bold mb-4 text-gray-800">Step 1: Select Your Insurance Company</h2>
            ...
            <div className="mb-6">
              ...
              <SearchableSelect
                ...
                placeholder="Search and select your insurance company..."
                required
              />
            </div>
          ...
          /* Step 2: Premium Payment Form */
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-800">Step 2: Enter Premium Details</h2>
              <button
                onClick={() => {
                  setCurrentStep(1);
                  setOperatorSelected(false);
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                {/* Fallback Consumer Number field - only show if no dynamic fields */}
                ...
                {/* Premium Details Display */}
                {billDetails && (
                  <div className="md:col-span-2 bg-green-50 border border-green-200 rounded-lg p-4">
                    <h3 className="font-semibold text-green-800 mb-3">📋 Premium Details</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      ...
                      <div>
                        <span className="font-medium text-gray-700">Premium Period:</span>
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="Enter premium amount to pay"
                    min={operators.find(op => op.id === selectedOperator)?.min_amount || 1}
              ...
              >
                {loading ? '⏳ Processing Payment...' : `🚀 Pay Insurance Premium - ₹${amount || '0'}`}
              </button>
