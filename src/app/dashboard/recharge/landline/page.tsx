...
export default function LandlineBillPage() {
  ...
  const fetchOperators = async () => {
    try {
      const res = await fetch('/api/recharge/operators?service_type=LANDLINE');
      const data = await res.json();
      if (data.success) {
  ...
  const fetchBill = async () => {
    if (!consumerNumber || !selectedOperator) {
      setMessage('Please enter landline number and select operator');
    ...
    try {
      ...
      if (data.success) {
        ...
        if (status === 'SUCCESS') {
          ...
          const finalSuccessMsg = `✅ ${cleanMessage}! Your landline bill payment for ${consumerNumber} was successful.${operatorRef ? `\nRef: ${operatorRef}` : ''}`;
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8 max-w-7xl">
        <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 text-gray-800">☎️ Landline Bill Payment</h1>

        {/* Header */}
        <div className="mb-4 sm:mb-6">
          <div className="bg-green-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold text-sm sm:text-base text-center">
            ☎️ LANDLINE BILL PAYMENT
          </div>
        </div>
        ...
        {currentStep === 1 ? (
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            <h2 className="text-xl font-bold mb-4 text-gray-800">Step 1: Select Your Landline Provider</h2>
            
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Select Landline Provider
              </label>
              <SearchableSelect
                ...
                placeholder="Search and select your landline provider..."
                required
              />
            </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                ...
                {dynamicFields.length === 0 && (
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      placeholder="Enter your landline number (with STD code)"
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                      required
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Include STD code (e.g., 01123456789 for Delhi)
                    </p>
                  </div>
                )}
              ...
              >
                {loading ? '⏳ Processing Payment...' : `🚀 Pay Landline Bill - ₹${amount || '0'}`}
              </button>
